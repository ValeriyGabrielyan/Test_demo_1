...
Процедура КалендарьПриВыводеПериода(Элемент, ОформлениеПериода)
	...
	Для Каждого СтрокаОформленияПериода Из ОформлениеПериода.Даты Цикл
		...
		Если ЦветОформленияДня = Неопределено Тогда
			ЦветОформленияДня = ОбщегоНазначенияКлиент.ПолучитьЦвет("РабочийЦвет");
		КонецЕсли;
...
&НаКлиенте
Процедура ПослеВыбораИзСписка(ВыбранныйЭлемент,ВыделенныеДаты) Экспорт
...
Процедура ПеренестиДень(Команда)
	...
	ПараметрыВыбораДаты.Вставить("ПоясняющийТекст",				ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку(
																НСтр("ru = 'Выберите дату, на которую будет осуществлен перенос дня %1 (%2)'"), 
																Формат(ДатаПереноса, "ДФ='д ММММ'"), 
																ВидДня));
	
	ОткрытьФорму("ОбщаяФорма.ВыборДаты", ПараметрыВыбораДаты, ЭтаФорма, УникальныйИдентификатор);
...
Процедура ИзменитьВидыДней(ДатыДней, ВидДня)
	...
	ВидыДнейСоответствие = ОбщегоНазначенияКлиентСервер.СоответствиеПоФиксированномуСоответствию(ВидыДней);
...
Процедура ПеренестиВидДня(ДатаПереноса, ДатаНазначения)
	...
	ПереносыДнейСоответствие = ОбщегоНазначенияКлиентСервер.СоответствиеПоФиксированномуСоответствию(ПереносыДней);
...
Процедура ЗаполнитьПредставлениеПереносов(Форма)
	...
	Для Каждого КлючИЗначение Из Форма.ПереносыДней Цикл
		...
		Форма.СписокПереносов.Добавить(ДатаИсточник, ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если НЕ РольДоступна("ПолныеПрава") ИЛИ НЕ РольДоступна("АдминистраторСистемы") Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не достаточно прав для записи объекта");
		Возврат;
	КонецЕсли;	
КонецПроцедуры
